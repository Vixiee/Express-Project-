  ...
  db.run(
    ...
    function (err) {
      if (err) {
        return res
          .status(500)
          .json({ error: "Грешка при създаване на задача." });
      }
      res.status(201).json({
        id: this.lastID,
        title: title.trim(),
...
app.put("/tasks/:id", (req, res) => {
  ...
  db.get("SELECT * FROM tasks WHERE id = ?", [id], (err, row) => {
    ...
    if (!row) {
      return res
        .status(404)
        .json({ error: `Задача с id ${id} не е намерена.` });
    }
    ...
    if (!newTitle) {
      return res
        .status(400)
        .json({ error: 'Полето "title" не може да бъде празно.' });
    }
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          return res
            .status(500)
            .json({ error: "Грешка при обновяване на задача." });
        }
        res.json({
          id: Number(id),
          title: newTitle,
